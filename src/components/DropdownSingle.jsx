...
const DropdownSingle = ({ values, setValues, className }) => {
    ...
    return (
        <div className = {`${styles.dropdown} ${className}`}>
            ...
            <div className = {`${styles.container} h-100`}>
                <div className = {`${styles.selectButton} px-4 d-flex align-items-center h-100`} onClick = {handleOpen}>
                    <span className = {styles.icon}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="29.572" height="19.715" viewBox="0 0 29.572 19.715"><path id="Icon_material-sort" data-name="Icon material-sort" d="M4.5,28.715h9.857V25.429H4.5ZM4.5,9v3.286H34.072V9Zm0,11.5H24.215V17.215H4.5Z" transform="translate(-4.5 -9)"/></svg>
                    </span>
                    <h4 className = "fw-500 ms-3">{values.find(value => value.active === true).label}</h4>
                ...
                <ul className = {`${styles.options} ${open ? 'd-block' : 'd-none'} mt-3`}>
                    { values?.map((value, index) => {
                        return (
                            <li className = {`${styles.option} d-flex py-3 px-3 align-items-center`} key = {index} onClick = {() => handleActive(value.id)}>
